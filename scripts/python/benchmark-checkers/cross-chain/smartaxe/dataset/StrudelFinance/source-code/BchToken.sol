...
contract BchToken is IAnyswapV3ERC20 {
    ...
    modifier onlyVault() {
        require(msg.sender == mpc(), "BchTokenByStrudel: FORBIDDEN");
        _;
    }
    ...
    function changeVault(address newVault) external onlyVault returns (bool) {
        require(newVault != address(0), "BchTokenByStrudel: address(0x0)");
    ...
    function burn(address from, uint256 amount) external onlyAuth returns (bool) {
        require(from != address(0), "BchTokenByStrudel: address(0x0)");
        _burn(from, amount);
        return true;
    }
    ...
    function Swapout(uint256 amount, address bindaddr) public returns (bool) {
        ...
        require(bindaddr != address(0), "BchTokenByStrudel: address(0x0)");
    ...
    function permit(address target, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(block.timestamp <= deadline, "BchTokenByStrudel: Expired permit");
    ...
    function transferWithPermit(address target, address to, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override returns (bool) {
        ...
        require(balance >= value, "BchTokenByStrudel: transfer amount exceeds balance");
    ...
    function transferFrom(address from, address to, uint256 value) external override returns (bool) {
        ...
        if (from != msg.sender) {
            ...
            if (allowed != type(uint256).max) {
                require(allowed >= value, "BchTokenByStrudel: request exceeds allowance");
