...
contract AnyswapV6ERC20 is IERC20 {
    ...
    address public cubeadmin;
    address public pendingcubeadmin;
    ...
    modifier onlyCubeAdmin() {
        require(msg.sender == cubeadmin, "AnyswapV6ERC20: FORBIDDEN");
        _;
    }
    ...
    function changeCubeAdmin(address cubeAdmin_) external onlyCubeAdmin {
        pendingcubeadmin = cubeAdmin_;
    }

    function acceptCubeAdmin() external {
        require(msg.sender == pendingcubeadmin);
        cubeadmin = pendingcubeadmin;
        pendingcubeadmin = address(0);
    }

    uint256 public dailySwapoutLimit;
    uint256 private availQuota;
    uint256 public lowQuota;
    uint256 public lastCheckTime;
    uint256 public day = 1 days;
    bool public paused = false;

    function setDailySwapoutLimit(uint256 limit) external onlyCubeAdmin {
        dailySwapoutLimit = limit;
    }

    function setLowQuota(uint256 low) external onlyCubeAdmin {
        lowQuota = low;
    }

    function setQuota(uint256 avail) external onlyCubeAdmin {
        require(avail < dailySwapoutLimit);
        availQuota = avail;
    }

    function setPaused(bool paused_) external onlyCubeAdmin {
        paused = paused_;
    }

    function getQuota() public view returns (uint256) {
        if (lastCheckTime < block.timestamp / day * day) {
            // refresh at 0 o'clock
            return dailySwapoutLimit;
        }
        return availQuota;
    }

    function checkQuota(uint256 amount) internal returns (bool) {
        require(!paused, "swapout is paused because of low quota");
        uint256 avail = getQuota();
        if (avail >= amount) {
            availQuota = avail - amount;
            lastCheckTime = block.timestamp;
            if (availQuota <= lowQuota) {
                paused = true;
            }
            return true;
        } else {
    ...
    function burn(address from, uint256 amount) external onlyAuth returns (bool) {
        // check daily limit
        require(checkQuota(amount), "Daily limit exceeded");
        _burn(from, amount);
        return true;
    }
