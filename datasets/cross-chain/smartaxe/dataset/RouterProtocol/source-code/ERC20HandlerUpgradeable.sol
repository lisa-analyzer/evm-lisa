    ...
    ILiquidityPool
{
    ...
    function changePrecision(address token, uint8 chainId, uint256 tokenAmount) public view returns (uint256) {
    ...
    function executeProposal(
        SwapInfo memory swapDetails,
        bytes32 resourceID
    ) public virtual override onlyBridgeOrSequencer returns (address settlementToken, uint256 settlementAmount) {
    ...
    function handleUsdcBurn(uint256 amount, uint8 destChainId, uint64 depositNonce) internal {
    ...
    function withdrawNative(address recipient, uint256 amount, bool withdrawAll) external onlyRole(DEFAULT_ADMIN_ROLE) {
