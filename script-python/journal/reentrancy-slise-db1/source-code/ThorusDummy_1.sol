...
/**
 * @title ThorusDummy
 * @dev Dummy contract to make ThorusMaster believe it is minting and sending reward tokens.
 * To be used on chains where the reward token is bridged, not mintable and should be provided externally.
 *
 * balanceOf() returns how many tokens ThorusMaster has promised to give out
 * transfer() giving out tokens on behalf of ThorusMaster
 * mint() reserves tokens for ThorusMaster to give out
 *
 * These are all the functions that ThorusMaster uses from the reward token.
 */
