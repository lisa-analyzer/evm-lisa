 ...
 */
public class EVMUncheckedExternalInfluenceAbstractSemanticsTest extends EVMBytecodeAnalysisExecutor {
	private static final Logger log = LogManager.getLogger(EVMUncheckedExternalInfluenceAbstractSemanticsTest.class);
	...
	private static CronConfiguration createConfiguration(String testDir, String subDir, String programFile,
		...
		conf.abstractState = new SimpleAbstractState<MonolithicHeap, TaintAbstractDomain,
				TypeEnvironment<InferredTypes>>(
						new MonolithicHeap(), new UncheckedExternalInfluenceAbstractDomain(),
	...
	@Test
	public void testUncheckedExternalInfluence01() throws AnalysisSetupException, IOException {
		// Vulnerability present
		CronConfiguration conf = createConfiguration("taint/test-unchecked-external-influence", "test01", "test01.sol",
				false);
		JumpSolver checker = new JumpSolver();
		conf.semanticChecks.add(checker);
		conf.semanticChecks.add(new UncheckedExternalInfluenceChecker());
		conf.abstractState = new SimpleAbstractState<>(new MonolithicHeap(),
				new UncheckedExternalInfluenceAbstractDomain(),
				new TypeEnvironment<>(new InferredTypes()));
		perform(conf);

		int warnings = MyCache.getInstance().getUncheckedExternalInfluenceWarnings(checker.getComputedCFG().hashCode());
		log.warn("Unchecked external influence warning: {}", warnings);

		assert warnings == 1;
	}

	@Test
	public void testUncheckedExternalInfluence02() throws AnalysisSetupException, IOException {
		// Vulnerability NOT present
		CronConfiguration conf = createConfiguration("taint/test-unchecked-external-influence", "test02", "test02.sol",
				false);
		JumpSolver checker = new JumpSolver();
		conf.semanticChecks.add(checker);
		conf.semanticChecks.add(new UncheckedExternalInfluenceChecker());
	...
	@Test
	public void testUncheckedExternalInfluence03() throws AnalysisSetupException, IOException {
		// Vulnerability NOT present
		CronConfiguration conf = createConfiguration("taint/test-unchecked-external-influence", "test03", "test03.sol",
				false);
		JumpSolver checker = new JumpSolver();
		conf.semanticChecks.add(checker);
		conf.semanticChecks.add(new UncheckedExternalInfluenceChecker());
	...
	@Test
	public void testUncheckedExternalInfluence04() throws AnalysisSetupException, IOException {
		// Vulnerability present
		CronConfiguration conf = createConfiguration("taint/test-unchecked-external-influence", "test04", "test04.sol",
				false);
		JumpSolver checker = new JumpSolver();
		conf.semanticChecks.add(checker);
		conf.semanticChecks.add(new UncheckedExternalInfluenceChecker());
