...
public class LocalDependencyCheckerNewVersion implements
		SemanticCheck<SimpleAbstractState<MonolithicHeap, TaintAbstractDomain, TypeEnvironment<InferredTypes>>> {

	private static final Logger log = LogManager.getLogger(LocalDependencyCheckerNewVersion.class);
	 ...
	 */
	public LocalDependencyCheckerNewVersion(SmartContract contract, CustomPolicy policy) {
		...
		if (node instanceof Log) {
			...
			for (AnalyzedCFG<SimpleAbstractState<MonolithicHeap, TaintAbstractDomain,
				...
				try {
					...
					log.error("(LocalDependencyCheckerNewVersion): {}", e1.getMessage());
				}
