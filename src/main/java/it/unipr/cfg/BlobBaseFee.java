...
import it.unipr.analysis.operator.BlobBaseFeeOperator;
...
public class BlobBaseFee extends Statement {

	/**
	 * Builds a BlobBaseFee opcode happening at the given source location.
	 * 
	 * @param cfg      the cfg that this statement belongs to
	 * @param location the location where this statement is defined within the
	 *                     program
	 */
	public BlobBaseFee(CFG cfg, CodeLocation location) {
		super(cfg, location);
	}
	...
	public <A extends AbstractState<A>> AnalysisState<A> forwardSemantics(AnalysisState<A> entryState,
		...
		return entryState.smallStepSemantics(
				new it.unive.lisa.symbolic.value.UnaryExpression(Untyped.INSTANCE, DummyConstant.INSTANCE,
						BlobBaseFeeOperator.INSTANCE, getLocation()),
				this);
	}
